...
export class DiscordInviteWorker {
  ...
  private registerEvents() {
    ...
    this.client.on(Events.GuildMemberAdd, (member) => {
      if (!this.configuredGuildIds.has(member.guild.id) || member.user.bot)
        return;
      this.enqueue(member.guild.id, () => this.handleMemberAdd(member));
    });
    this.client.on(Events.InviteCreate, (invite) => {
      if (!invite.guild || !this.configuredGuildIds.has(invite.guild.id))
        return;
  ...
  private async seedConfiguredGuilds() {
    ...
    for (const guildId of this.config.discordGuildIds) {
      try {
        ...
        await this.repository.replaceSnapshots(guild.id, [
          ...snapshots.values(),
        ]);
  ...
  private async handleInviteCreate(invite: Invite) {
    ...
    state.recentDeleted = state.recentDeleted.filter(
      (entry) => entry.code !== snapshot.code,
    );
  ...
  private async handleInviteDelete(invite: Invite) {
    ...
    if (!state || !invite.guild) return;
    const previous =
      state.snapshots.get(invite.code) ?? snapshotFromInvite(invite);
  ...
  private async handleMemberAdd(member: GuildMember) {
    ...
    if (!state) {
      throw new Error(
        `Servidor ${member.guild.id} ainda não possui baseline de convites.`,
      );
    }

    const now = Date.now();
    const current = await this.fetchSnapshots(member.guild);
    const pendingCredits = discardExpiredCredits(state.pendingCredits, now);
    const recentDeleted = state.recentDeleted.filter(
    ...
    const detectedCredits = [...pendingCredits, ...detected.credits];
    const nextRecentDeleted = detected.consumedDeletedCode
      ? recentDeleted.filter(
          (invite) => invite.code !== detected.consumedDeletedCode,
        )
      : recentDeleted;
    const [credit, ...remainingCredits] = detectedCredits;

    // Persist both sides before consuming the credit in memory. If Supabase is
    // briefly unavailable, the retry below recomputes the same transition from
    // the old state instead of assigning this invite to the next member.
    await this.repository.replaceSnapshots(member.guild.id, [
      ...current.values(),
    ]);
    const result = await this.repository.recordJoin({
      ...
      inviteeAvatarUrl: member.user.displayAvatarURL({
        extension: "png",
        size: 128,
      }),
      inviteeAccountCreatedAt: new Date(
      ...
      details: credit
        ? {
            ...
            queued_credits_after: remainingCredits.length,
          }
        : {
    ...
    state.pendingCredits = remainingCredits;
    state.recentDeleted = nextRecentDeleted;
  ...
  private async fetchSnapshots(guild: Guild) {
    ...
    for (let attempt = 1; attempt <= 3; attempt += 1) {
      try {
        ...
        return new Map(
          invites.map((invite) => [invite.code, snapshotFromInvite(invite)]),
        );
      } catch (error) {
        lastError = error;
  ...
  private enqueue(guildId: string, task: () => Promise<void>) {
    ...
    const next = previous
      .catch(() => undefined)
      .then(() => retryWorkerTask(task))
      .catch((error) => {
        this.setError(error);
        log("error", "guild_task_failed", {
          guildId,
          message: safeMessage(error),
        });
      })
...
export async function retryWorkerTask(
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      ...
      lastError = error;
      if (attempt < attempts) await wait(attempt * 350);
    }
  }

  throw lastError instanceof Error
    ? lastError
    : new Error("A tarefa do worker falhou após novas tentativas.");
}

...
describe("Robux pricing", () => {
  it("charges R$ 40,00 for 1.000 Robux", () => {
    expect(calculateRobuxPriceCents(1_000)).toBe(4_000);
  });

  it("uses the configured minimum quantity", () => {
    expect(MINIMUM_ROBUX_QUANTITY).toBe(100);
    expect(calculateRobuxPriceCents(MINIMUM_ROBUX_QUANTITY)).toBe(400);
  ...
  it("rejects invalid and oversized quantities", () => {
    ...
    expect(calculateRobuxPriceCents(MAXIMUM_ROBUX_QUANTITY)).toBe(2_000_000);

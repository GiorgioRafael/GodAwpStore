-- GWStore Robux price update: R$ 42,00 per 1.000 Robux.
-- Preserve the rate recorded on historical orders so paid or pending checkouts
-- created under the previous R$ 35,00 rate remain valid and auditable.
begin;
...
alter table public.robux_orders
  add column if not exists price_per_thousand_cents integer;

update public.robux_orders
set price_per_thousand_cents = 3500
where price_per_thousand_cents is null;

alter table public.robux_orders
  alter column price_per_thousand_cents set default 4200,
  alter column price_per_thousand_cents set not null,
  drop constraint if exists robux_orders_amount_matches_quantity,
...
alter table public.robux_orders
  add constraint robux_orders_price_per_thousand_cents_valid
    check (price_per_thousand_cents in (3500, 4200)),
  add constraint robux_orders_amount_matches_quantity
    check (
      amount_cents = (
        (robux_quantity::bigint * price_per_thousand_cents + 999) / 1000
      )
    );
...
begin
  ...
  v_amount_cents := ((p_robux_quantity::bigint * 4200 + 999) / 1000);

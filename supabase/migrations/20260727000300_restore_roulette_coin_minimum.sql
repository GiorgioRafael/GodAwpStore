-- Restore the one-coin floor for roulette coin purchases.
-- LivePix charges a flat percentage per Pix receipt with no fixed component,
-- so a R$ 1,00 ticket carries the same margin as a R$ 100,00 one and there is
-- no cost reason to force a larger deposit.
...
comment on function public.start_roulette_coin_purchase(text, integer) is
  'Opens or reuses the LivePix charge that buys 1 to 100 roulette coins at R$ 1,00 each.';
